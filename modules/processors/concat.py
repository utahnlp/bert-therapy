  ...
  for file in files:
    ...
    with open(file) as json_file:
      for (idx, line) in enumerate(json_file):
        ...
        # for the start and end tokens of context, and end token of utterance
        ...
        for context_message in context:
          ...
          length += len(lru_tokenize(context_utterance))
          if length > max_length:
            break
          current_context.insert(0, context_utterance)
        if len(current_context) > 0:
          dataset['context'].append('. '.join(current_context))
        else:
          continue
